...
class UserLoginView(APIView):
    ...
    def post(self, request):
        ...
        try:
            user_login_func(request=request, email=serializer.validated_data["email"])
        except PermissionError:
            return Response(
                data={"detail": _("Please try again later")},
...
class UserVerifyView(APIView):
    ...
    def post(self, request):
        ...
        vd = serializer.validated_data
        try:
            data = user_verify_func(request=request, code=vd["code"], email=vd["email"])
        except ValueError:
            return Response(
                data={"detail": _("Invalid code")}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ResendVerifyMessage(APIView):
    ...
    def post(self, request):
        ...
        serializer.is_valid(raise_exception=True)
        try:
            user_resend_func(request=request, email=serializer.validated_data["email"])
        except PermissionError:
...
class JwtVerifyView(APIView):
    ...
    def post(self, request):
        ...
        if check is False:
            return Response(
                data={"token": _("Invalid token")}, status=status.HTTP_400_BAD_REQUEST
            )
        return Response(status=status.HTTP_200_OK)
