...
def check_validate_refresh_token(refresh_token: str) -> bool:
    try:
        user_login = UserLogin.objects.get(
            refresh_token=encrypt(data=str(refresh_token))
        )
...
def get_new_access_token(encrypted_refresh_token: ByteString, client_info: Dict) -> str:
    ...
    token = validate_refresh_token(
        refresh_token=decrypted_refresh_token, client_info=client_info
    )
...
def verify_token(token: Token, client_info: Dict) -> bool:
    if (
        ...
        or token["ip_address"] != client_info["ip_address"]
    ):
        return False
    return True
...
def black_refresh_token(encrypted_refresh_token: ByteString, client_info: Dict) -> None:
    ...
    token = validate_refresh_token(
        refresh_token=decrypted_token, client_info=client_info
    )
